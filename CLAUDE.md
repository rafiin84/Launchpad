...
- **File storage** for anything the app itself needs to read back (Documents module, application document requests) goes through Zoho's **File Upload field** mechanism, not the Attachments API:
  ...
  - Activity images (`AddActivity.tsx`, `Activities.tsx`) still use `src/services/fileUpload.ts`, an **unsigned Cloudinary upload** (`VITE_CLOUDINARY_CLOUD_NAME` / `VITE_CLOUDINARY_UPLOAD_PRESET` in `.env`), since those aren't stored as their own CRM records.
...
`My_Activities` and `Feed_Submissions` both carry the same 7 extra fields for these: `Post_Type`, `Video_URL`, `Link_URL`, `Location_Name`, `Location_Coords`, `Poll_Data`, `Document_Ref` (see `FIELD_MAP` in `crmActivities.ts`). `Post_Type` picks the renderer in `ActivityCard`/`ActivityDetail.tsx`; records saved before it existed fall back to the old imageUrl/imageData inference. Documents reuse the Documents-page mechanism (`createCRMDocumentFromFile`, Zoho File Upload field) and store `{documentId, fileUploadId, fileName}` as JSON in `Document_Ref`; polls store only `{question, options}` in `Poll_Data` — **never vote tallies**.
