...
- **File storage** for anything the app itself needs to read back (Documents module, application document requests) goes through Zoho's **File Upload field** mechanism, not the Attachments API:
  ...
  - **Company logos live on `Founder_Companies` itself** via `Company_Logo`, a File Upload field — not Zoho's Record Image API, which the module used before. `uploadCompanyLogoFile`/`resolveCompanyLogoFileUrl` (`companyProfile.ts`) upload/read it the same way as every other File Upload field in this app. Reading always falls back to the old Record Image (`zohoGetRecordPhoto`/`portalGetRecordPhoto`) if the new field is empty, so a company that already had a logo set the old way doesn't lose it — but every new upload goes through `Company_Logo` only; nothing writes to Record Image anymore.
