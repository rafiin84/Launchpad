...
import {
  ...
  PieChart, LayoutDashboard, ShieldCheck, ArrowRight,
} from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import type { UserRole } from '../types';
import { cn } from '../lib/cn';
import { redirectToZoho, redirectToPortal, savePendingRole, savePortalLoginEmail } from '../services/oauth';
...
export default function Login() {
  ...
  function handleFounderLogin(e: React.FormEvent) {
    e.preventDefault();
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        <div className="text-center mb-12 max-w-xl">
          ...
          <p className="text-gray-500 text-base leading-relaxed">
            {showEmailInput ? 'Enter your email to sign in as Founder.' : 'Select your role to sign in.'}
          </p>
        </div>
            ...
            <form onSubmit={handleFounderLogin} className="space-y-4">
              <div className="bg-white border-2 border-indigo-100 rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-gray-900">Founder Sign In</h3>
                    <p className="text-xs text-gray-500">Enter your portal invitation email</p>
                  </div>
                </div>
                <input
                  type="email"
                ...
                <button
                  type="submit"
                  className="w-full mt-3 flex items-center justify-center gap-2 px-4 py-3 bg-black text-white text-sm font-semibold rounded-xl hover:bg-gray-800 transition-colors"
                >
                  Continue to Portal
                  <ArrowRight size={14} />
                </button>
